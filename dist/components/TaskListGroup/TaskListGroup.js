 ...
 */
const TaskListGroup = function TaskListGroup({ id, label, content, items = [], ordered = false, attributes = {}, headingLevel = 2, }) {
    let allSections = [];
    ...
    const numComplete = (allSections.filter(({ status }) => status === TaskList_type_1.TaskListStatus.Complete).length);
    const firstIncomplete = allSections.find(({ status }) => status !== TaskList_type_1.TaskListStatus.Complete);
    return (react_1.default.createElement(react_1.default.Fragment, null,
        label && (react_1.default.createElement(Heading_1.default, { className: "ds_task-list-status-heading", level: headingLevel }, (0, htmlToReact_1.default)(label, false))),
        ...
        react_1.default.createElement(WrapperTag_1.default, { tag: ordered ? 'ol' : 'ul', className: (0, classNames_1.default)('ds_task-list-group', ordered ? 'ds_task-list-group--ordered' : ''), id: id, ...attributes }, items.map((item, index) => {
            const key = `${id}-${index}`;
